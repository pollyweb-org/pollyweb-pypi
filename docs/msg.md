...
| `Algorithm` | `str` | — | `""` | Signature algorithm for the message, using DKIM-style values such as `ed25519-sha256` or `rsa-sha256`. `msg.sign()` fills this automatically when omitted. Domain senders must leave it empty on the wire because receivers infer the algorithm from DKIM. |
