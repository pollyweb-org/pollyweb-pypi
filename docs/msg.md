...
| `Body` | `dict` | — | `{}` | Arbitrary JSON-serialisable payload. Any extra keyword arguments passed to `Msg()` that are not recognised field names are merged into `Body` automatically. |
...
Extra keyword arguments are merged into `Body` at construction time, so `Msg(To="a.com", Subject="X@H", text="hi")` is equivalent to `Msg(To="a.com", Subject="X@H", Body={"text": "hi"})`. When both `Body` and extra kwargs are given, the kwargs are merged on top of `Body`.
