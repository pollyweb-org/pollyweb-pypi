...
| `Algorithm` | `str` | — | `""` | Signature algorithm for the message, using DKIM-style values such as `ed25519-sha256` or `rsa-sha256`. `msg.sign()` fills this automatically when omitted. Domain-signed messages may leave it empty on the wire because receivers can infer the algorithm from DKIM. |
