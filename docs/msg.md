...
domain = pw.Domain(Name="sender.dom", PrivateKey=private_key, DKIM="pk1")
...
msg = pw.Msg(From="sender.dom", To="receiver.dom", Subject="Hello@Host", DKIM="pk1", Body={"text": "hi"})
...
| `DKIM` | `str` | — | `""` | Name of the sender's public key in DNS (e.g. `pk1` → `pk1._domainkey.pw.sender.dom`). Set by `Domain.sign()` if omitted. |
