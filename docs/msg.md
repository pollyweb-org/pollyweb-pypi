...
`Msg.verify()` infers the signature algorithm from the verified public key or, for domain senders, from the sender DKIM key type in DNS. PollyWeb currently supports Ed25519 (`k=ed25519`) and RSA SHA-256 (`k=rsa`).
...
**Algorithm agility** — PollyWeb treats the verified public key as the source of truth for signature verification. For domain messages, receivers infer the algorithm from the published DKIM key type (`k=`) instead of trusting a serialized message header.
...
**`From` and `Selector` are optional at construction** — `From` and `Selector` default to `""`, so a `Msg` can be built as a draft before the sender or DNS selector is known. `From` may also be set explicitly to `Anonymous`, a domain string, or a UUID. Before a message is signed or validated, `From` must be populated; [`domain.sign()`](domain/sign.md) fills `From`, derives `Selector`, and signs with the domain's Ed25519 key. `verify()` requires `Selector` only when it must resolve the sender key from DNS.
