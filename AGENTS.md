...
## Learnings
- GitHub Actions workflow conditions for optional follow-up steps are safer when they avoid direct `secrets.*` checks in step-level `if` expressions; pass the secret through `env` and skip inside the shell so the workflow still schedules and a successful publish does not fail at parse time.
...
- Wrapped synchronous inbox replies should be parsed in this package, not in thin clients: `Msg.parse(..., sync_response = True)` should validate the outer `Request`/`Response`/`Meta` envelope and unwrap the nested `Response` message before normal verification.
...
- `Msg.parse()` should be the shared entrypoint for inbound AWS wrappers: when a message is nested inside EventBridge, SNS, SQS, API Gateway, Kinesis, or common Lambda / Step Functions payload fields such as `payload`, `Payload`, `message`, or `raw_payload`, teach the library to unwrap it there so service repos can call `pw.Msg(event)` instead of hand-parsing transport envelopes.
